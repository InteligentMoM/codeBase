...
def main(wav_path, embedding_per_second=1.0, overlap_rate=0.5):
    ...
    dataset = pd.DataFrame(data, columns=['Speaker','Start_Time','End_Time'])
    dataset.to_csv("Speaker_time.csv")
...
if __name__ == '__main__':
    data = pd.read_csv("Audio_File_Name.csv")
    audio_file = data['File_Name'][0]
    main(audio_file, embedding_per_second=1.2, overlap_rate=0.4)
